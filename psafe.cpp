...
std::optional<CryptoPP::SecBlock<byte>> FieldLoader::LoadNext() {
  ...
  if (dataLength > 11) {
    ...
    decryptor_.ProcessData(blocks.BytePtr() + Twofish::BLOCKSIZE,
                           &remainingBlocks[0], remainingBlocks.size());
  }
...
PasswordSafe::Load(std::istream &source,
  ...
  if (!SHA256().VerifyDigest(&stretchedKeyHash[0], stretchedKey,
  ...
  cbc.SetKeyWithIV(randomKeyK, randomKeyK.SizeInBytes(), &initialValue[0],
                   initialValue.size());
