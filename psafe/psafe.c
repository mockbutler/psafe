...
*/
void sha256_block32(const u8* in, u8* out)
{
    gcry_md_hd_t hd;
    gcry_error_t gerr;
 ...
 */
void extract_random_key(const u8* stretchkey,
                        const u8* fst,
                        const u8* snd,
                        u8* randkey)
{
...
void printhex(FILE* f, u8* ptr, unsigned cnt)
{
    unsigned i;
    for (i = 0; i < cnt; i++)
...
void pws(FILE* f, u8* bp, size_t len)
{
    mbstate_t state;
